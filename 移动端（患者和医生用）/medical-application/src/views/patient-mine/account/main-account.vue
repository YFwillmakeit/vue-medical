...
export default {
  ...
  methods: {
    ...
    sure() {
      ...
      this.$axios
        .post(
          this.$api.patient.recharge,
          JSON.stringify({ acount: this.recharge, patientId: 1 })
        )
        .then(response => {
          var result = response.data;
          if (result.code === "200") {
    ...
    pExplain() {
      this.$router.push("/mine/account/explain");
    },
    pDetailed() {
      this.$router.push("/mine/account/detailed");
    },
    showPopup() {
      if (this.recharge == "") {
        this.headShow = true;
